...
export class InvoicePdfGenerator {
  ...
  static async createPdfFromFhirInvoice(
    ...
    const lineItems: LineItemData[] = fhirInvoice.lineItem?.map((item) => ({
      ...
      totalPrice: item.priceComponent?.[0]?.amount?.value || 0,
    })) || [];

    // Extract totals
    const totalComponent = fhirInvoice.totalPriceComponent?.find((c) => c.type === 'total');
    ...
    const invoiceData: InvoicePdfData = {
      invoiceNumber: fhirInvoice.number || 'INV-UNKNOWN',
      invoiceDate: fhirInvoice.date || new Date().toISOString(),
      dueDate: fhirInvoice.dueDate,
      ...
      total: totalComponent?.amount?.value || 0,
      currency: fhirInvoice.currency || 'IDR',
