...
export default function ClinicCalendarOptimized({ 
  onDateSelect, 
  onAppointmentClick, 
  selectedDoctor,
  ...
  // Load appointments for the current month view - optimized with stable dependencies
  const loadAppointments = useCallback(async () => {
    ...
    try {
      ...
      // Use ref value to avoid dependency changes
      if (selectedDoctorRef.current) {
        query = query.eq("doctor_id", selectedDoctorRef.current);
      }
  ...
  // Separate effect for loading appointments when month changes
  useEffect(() => {
    void loadAppointments();
  }, [loadAppointments]);

  // Separate effect for handling doctor filter changes
  useEffect(() => {
    void loadAppointments();
  }, [selectedDoctor]); // This will trigger loadAppointments when doctor changes
  ...
  const getAppointmentStats = useCallback((date: Date) => {
